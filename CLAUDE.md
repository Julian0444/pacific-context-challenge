...
Scenario entry points (post UI-C). The three base stories (Permission Wall / VP Deal View / Stale Detection) live as dual-action cards in the Single empty state (primary `Run in Single` + secondary `Open in Compare →`) and as single-click preview cards in the Compare empty state. The shortcut rows under the search bar are deduplicated: the "Single" row keeps `DD risks` (vp) and `IC memo` (partner) — distinct queries, not base-story duplicates — and the "Compare" row keeps `Stale detection →` (partner, IC + LP query; formerly "Partner view ↔"). Removed buttons: `ARR growth` (Single row), `Analyst wall ↔` and `VP deal view ↔` (Compare row) — all three duplicated onboard-card queries. Total scenario entry points visible on initial Single open: 6 (3 empty-state cards + 2 Single row + 1 Compare row).
